...
#define NUM_BUY_OPTIONS_PER_THREAD 2 // Numero de buy options que tratará cada thread antes de la reducción

#define NUM_THREADS NUM_BUY_OPTIONS / NUM_BUY_OPTIONS_PER_THREAD // El número mínimo de threads es 32 (por el tamaño de warp) y el maximo 1024.
...
__global__ void KernelMarketplace(unsigned int *total_buy_options, unsigned int *best_buy_options)
{
    ...
    unsigned int global_thread_buy_option = ( blockIdx.x * (blockDim.x * 2) + (thread_id * 2)) * ELEMENTS_PER_BUY_OPTION;

    unsigned int first_store_id = total_buy_options[global_thread_buy_option + STORE_ID_OFFSET];
    unsigned int first_price = total_buy_options[global_thread_buy_option + PRICE_OFFSET];
    unsigned int second_store_id = total_buy_options[global_thread_buy_option + STORE_ID_OFFSET + ELEMENTS_PER_BUY_OPTION];
    unsigned int second_price = total_buy_options[global_thread_buy_option + PRICE_OFFSET + ELEMENTS_PER_BUY_OPTION];
